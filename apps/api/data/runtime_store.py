...
_shadow_prediction_jobs = {}
_latest_shadow_prediction_job_id = None
_refresh_lock = False
_feature_store_lock = False
_shadow_prediction_lock = False
_cron_status = {
...
def acquire_shadow_prediction_lock():
    global _shadow_prediction_lock
    latest = get_latest_shadow_prediction_job()
    if latest.get("status") != "running":
        _shadow_prediction_lock = False
    if _shadow_prediction_lock:
        return False
    if latest.get("status") == "running":
        return False
    _shadow_prediction_lock = True
    return True


def release_shadow_prediction_lock():
    global _shadow_prediction_lock
    _shadow_prediction_lock = False
...
def start_shadow_prediction_job(job_id):
    global _latest_shadow_prediction_job_id
    job = {
        "job_id": job_id,
        "status": "running",
        "started_at": _now_iso(),
    ...
    _shadow_prediction_jobs[job_id] = job
    _latest_shadow_prediction_job_id = job_id
    return job


def update_shadow_prediction_job(job_id, **fields):
    if not job_id:
        return get_latest_shadow_prediction_job()
    job = _shadow_prediction_jobs.get(job_id) or start_shadow_prediction_job(job_id)
    fields.setdefault("updated_at", _now_iso())
    job.update(fields)
    return _normalize_job(job)


def finish_shadow_prediction_job(job_id, result):
    job = update_shadow_prediction_job(job_id)
    if not job:
        return None
    job["status"] = "success"
    job["finished_at"] = _now_iso()
    ...
    job["error"] = None
    job["duration_ms"] = result.get("duration_ms") if isinstance(result, dict) else _duration_ms(job.get("started_at"), job.get("finished_at"))
    release_shadow_prediction_lock()
    return job


def fail_shadow_prediction_job(job_id, error):
    job = update_shadow_prediction_job(job_id)
    if not job:
        return None
    job["status"] = "error"
...
def get_shadow_prediction_job(job_id=None):
    if job_id:
        return _normalize_job(_shadow_prediction_jobs.get(job_id)) or {
            "job_id": job_id,
            "status": "idle",
            "started_at": None,
            "finished_at": None,
...
def get_latest_shadow_prediction_job():
    if not _latest_shadow_prediction_job_id:
        return {
            "job_id": None,
            "status": "idle",
            "started_at": None,
    ...
    return _normalize_job(_shadow_prediction_jobs.get(_latest_shadow_prediction_job_id)) or get_shadow_prediction_job(_latest_shadow_prediction_job_id)


def get_shadow_prediction_job_status(job_id=None):
    return get_shadow_prediction_job(job_id)


def reset_stale_jobs(force=False, stale_seconds=JOB_STALE_SECONDS):
    reset_count = 0
    for job in list(_refresh_jobs.values()) + list(_feature_store_jobs.values()) + list(_shadow_prediction_jobs.values()):
    ...
    if get_latest_feature_store_job().get("status") != "running":
        release_feature_store_lock()
    if get_latest_shadow_prediction_job().get("status") != "running":
        release_shadow_prediction_lock()
